...
class AppendProblem(ILPProblem):
    ...
    def get_answer(self, scene):
        answers = []
        # append an object
        scene = [s.color.name for s in scene]
        for color in colors:
            scene_app = [color] + scene
            # right_most object, left_most object
            answers.append(scene_app[-1])
            answers.append(scene_app[0])
        return answers

    def get_scenes_and_answers(self):
        scenes = []
        answers_list = []
        while len(scenes) < self.n:
            ...
            scenes.append(scene)
            answers_list.append(self.get_answer(scene))
...
class DeleteProblem(ILPProblem):
    ...
    def get_answer(self, scene):
        ...
        scene = [s.color.name for s in scene]
        for color in colors:
            scene_del = delete(color, scene)
            # right_most object, left_most object
            answers.append(scene_del[-1])
            answers.append(scene_del[0])
        return answers
